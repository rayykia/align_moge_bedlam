...
class MPT():
    def __init__(
        ...
        if detector_type == 'maskrcnn':
            self.detector = keypointrcnn_resnet50_fpn(
    ...
    def prepare_output_tracks(self, trackers):
        ...
        for frame_idx, tracks in enumerate(trackers):
            for d in tracks:
                person_id = int(d[4])
                # bbox = np.array([d[0], d[1], d[2] - d[0], d[3] - d[1]]) # x1,
                # y1, w, h
        ...
        for k in people.keys():
            people[k]['bbox'] = np.array(
                people[k]['bbox']).reshape(
                (len(
                    people[k]['bbox']), 4))
    ...
    def display_results(self, image_folder, trackers, output_file=None):
        ...
        for idx, (img_fname, tracker) in enumerate(
                zip(image_file_names, trackers)):
            ...
            for d in tracker:
                ...
                cv2.putText(
                    img, f'{d[4]}', (d[0] - 9, d[1] - 9), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0))
    ...
    def display_detection_results(
        ...
        for idx, (img_fname, dets) in enumerate(
                zip(image_file_names, detections)):
            print(img_fname)
            img = cv2.imread(img_fname)
            for d in dets:
                d = d.astype(np.int32)
    ...
    def detect(self, image_folder, output_file=None):
        ...
        if self.display:
            self.display_detection_results(
                image_folder, detections, output_file)
